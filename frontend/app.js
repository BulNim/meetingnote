...
function renderDetail(note) {
  ...
  const todoRows = String(note.todos || '').split(/\n|\\n/).map((line) => line.trim()).filter(Boolean).map((line) => {
    const [what = '미정', who = '미정', when = '미정'] = line.split('|').map((part) => part.trim() || '미정');
    return `<tr class="border-b border-slate-200/70 last:border-0 dark:border-slate-700"><td class="px-3 py-3">${escapeHtml(what)}</td><td class="px-3 py-3">${escapeHtml(who)}</td><td class="px-3 py-3">${escapeHtml(when)}</td></tr>`;
  }).join('');
  const todoTable = todoRows ? `<div class="mt-3 overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700"><table class="w-full min-w-[480px] text-left text-sm"><thead class="bg-emerald-100 text-emerald-900 dark:bg-emerald-950/60 dark:text-emerald-100"><tr><th class="px-3 py-2">할 일</th><th class="px-3 py-2">담당자</th><th class="px-3 py-2">기한</th></tr></thead><tbody>${todoRows}</tbody></table></div>` : '<p class="mt-3 text-sm">없음</p>';
  const todoSection = `<article class="border-l-4 border-emerald-500 rounded-2xl bg-slate-100/80 p-4 dark:bg-slate-800/80"><h3 class="font-semibold">할 일</h3>${todoTable}</article>`;
  renderShell(`<section class="rounded-3xl border border-white/70 bg-white/70 p-5 shadow-xl shadow-slate-200/50 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-black/20"><div class="flex items-start justify-between gap-4"><div><p class="text-sm text-slate-500">${formatLocalDate(note.met_at)} · ${escapeHtml(note.attendees || '참석자 미정')}</p><h2 class="mt-2 text-3xl font-semibold">${escapeHtml(note.title)}</h2></div><button id="closeDetail" class="rounded-xl px-3 py-2 text-sm hover:bg-slate-100 dark:hover:bg-slate-800">닫기</button></div><div class="mt-6 grid gap-3 md:grid-cols-3">${section('요약', note.summary, 'border-cyan-500')}${section('결정사항', note.decisions, 'border-orange-500')}${todoSection}</div><details class="mt-5 rounded-2xl bg-slate-100/70 p-4 dark:bg-slate-800/70"><summary class="cursor-pointer font-medium">받아쓴 본문 보기</summary><p class="mt-3 whitespace-pre-wrap text-sm">${escapeHtml(note.body)}</p></details><div class="mt-5 flex gap-3"><button id="editButton" class="rounded-2xl bg-slate-800 px-5 py-3 font-medium text-white dark:bg-white dark:text-slate-900">제목 수정</button><button id="detailDeleteButton" class="rounded-2xl bg-rose-600 px-5 py-3 font-medium text-white">삭제</button></div></section>`);
